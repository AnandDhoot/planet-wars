...
public class AllDistance
{
	...
	List<Distance> getDistancesToOtherPlanets(Planet planet)
	{
		return plDistances.get(planet.PlanetID());
	}

	void initialize(PlanetWars pw)
	{
		for (Planet planet : pw.Planets())
		{
			...
			for (Planet other : pw.Planets())
				if (other != planet)
					distancesToPlanet.add(new Distance(planet.PlanetID(), other
							.PlanetID(), pw.Distance(planet.PlanetID(),
							other.PlanetID())));
